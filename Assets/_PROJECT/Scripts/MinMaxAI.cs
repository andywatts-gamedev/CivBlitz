...
public class MinMaxAI : MonoBehaviour
{
    ...
    public (Vector2Int from, Vector2Int to) GetBestMove()
    {
        ...
        foreach (var unit in UnitManager.Instance.units
            .Where(u => u.Value.civ == TurnManager.Instance.aiCiv && u.Value.movement > 0))
        {
            var pos = unit.Key;
            var possibleMoves = HexGrid.GetValidMoves(pos, unit.Value.movement, UnitManager.Instance.tilemap);
